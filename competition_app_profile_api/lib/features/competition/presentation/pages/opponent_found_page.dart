...
class OpponentFoundArgs {
  ...
  /// The matched `MatchmakingEntry.queueId` — needed so this screen's
  /// countdown can hand off into `QuizPage` (which loads questions via
  /// `GetQuizQuestions(queueId: ...)`) once it reaches zero. Previously
  /// missing entirely, which meant there was no way to reach the quiz
  /// after a match was found (see `CompetitionRoutes`'s `quiz` route).
